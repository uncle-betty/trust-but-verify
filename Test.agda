...
open import SMT as S
  using (trueᶠ ; falseᶠ ; notᶠ ; iffᶠ ; appᶠ ; evalᶠ ; trustᶠ ; Holdsᶠ ; holdsᶠ)
...
open S.Rules env using (Atom ; atom ; mpᶠ ; assum ; assum-¬ ; clausi-f ; contra ; proofᶠ)
...
smt₁ : (x : Bool) → (T x → T x) × (T x → T x)
smt₁ x = proofᶠ (iffᶠ (appᶠ x) (appᶠ x)) (cvc4 x (holdsᶠ trueᶠ refl))
